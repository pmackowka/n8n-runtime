...
- **n8n czasem samo „normalizuje" parametry przy zapisie przez UI** — usuwa pola typu `resource`/`operation`, gdy pasują do wartości domyślnej (widziane na nodach Pushover, Date & Time, Telegram, Form Trigger `fieldType`, Google Sheets Tool, Marketstack Tool). Efekt: walidator może później zgłosić `INVALID_PARAMETER: missing discriminator`, mimo że node wcześniej działał bez zarzutu. Fix: ustaw `resource`/`operation`/`fieldType` jawnie przez `setNodeParameter` przy każdej edycji, nie tylko przy pierwszym tworzeniu.
- **Na node'ach Google Sheets Tool (`operation: update`) z nierozwiązanym `documentId`/`sheetName` (placeholder, brak realnego dokumentu) n8n potrafi przy zapisie przez UI wyzerować cały obiekt `columns` (resourceMapper: `mappingMode`, `value`, `schema`, `matchingColumns`)** — nie tylko pojedyncze pola jak w gotchu wyżej. Zaobserwowane po tym, jak użytkownik tylko otworzył/zapisał workflow w edytorze bez podłączania credentiala Google Sheets. Efekt: narzędzie AI traci całą konfigurację mapowania kolumn po cichu, bez ostrzeżenia. Dopóki dokument/arkusz nie jest realnie podłączony, traktuj `columns` jako coś, co trzeba weryfikować (i w razie potrzeby odtwarzać z poprzedniej wersji przez `get_workflow_version`/`get_workflow_versions_diff`) przy każdej kolejnej edycji tego workflow, nie tylko raz.
